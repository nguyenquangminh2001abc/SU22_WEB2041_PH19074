<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
      ...
      if (empty($errors)) {
          $conn = pdo_get_connection();
          $sql = "UPDATE khach_hang SET anh = '$anh' , ho_ten = '$ho_ten' , user_name = '$user_name' , email = '$email' , mat_khau = '$mat_khau', vai_tro = '$vai_tro' WHERE ma_khach_hang = $ma_khach_hang ";
          $stmt = $conn->prepare($sql);
          $stmt->execute();
...
<body>
    ...
    <main class="main">
        <section>
            ...
            <article>
                <button type="button" onclick=" window.location='add_khach_hang.php'"
        ...
        <section>
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select style="margin-bottom : 10px;" name="vai_tro" class="form-select">   
                        <?php $arr_vai_tro = ['Người Dùng','Quản Trị'] ; for($i = 0 ; $i < sizeof($arr_vai_tro) ; $i++) :  ?>
                         
                            <option <?= ($khach_hang['vai_tro'] == $i)? "selected":"" ?> value="<?= $i ?>"><?= $arr_vai_tro["$i"] ?></option>
                        
                        <?php endfor ?>
                    </select>
                    <span style="color:red;"><?= isset($errors['vai_tro']) ? $errors['vai_tro'] : '' ?></span><br>
                </div>
